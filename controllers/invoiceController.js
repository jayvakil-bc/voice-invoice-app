...
const sendInvoiceWithEmail = async (req, res) => {
    try {
        ...
        if (includePaymentLink && req.user) {
            try {
                const User = require('../models/User');
                const user = await User.findById(req.user.id);
                if (user && user.stripeAccountId && user.stripeOnboarded) {
                    paymentLink = await createPaymentLink(invoice, user.stripeAccountId);
...
const saveToGoogleDrive = async (req, res) => {
    try {
        ...
        // Generate PDF to temp file (reuse generateInvoicePDF logic)
        const tempDir = path.join(__dirname, '../uploads/temp');
        if (!fs.existsSync(tempDir)) {
            fs.mkdirSync(tempDir, { recursive: true });
        }
        ...
        // Generate PDF (simplified version - reuse from generateInvoicePDF)
        const brandColor = '#4542FF';
        const darkGray = '#333333';
        const mediumGray = '#666666';
