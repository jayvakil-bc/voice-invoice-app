...
router.put('/api/invoices/:id/regenerate', requireAuth, async (req, res) => {
    try {
...
- If only a total package price is mentioned, create a single line_item for the entire package`;
        ...
        invoice.originalTranscript = transcript;
        invoice.date = invoiceData.date || todayFormatted;
        invoice.dueDate = invoiceData.dueDate || invoiceData.due_date || dueDateFormatted;
        await invoice.save();
